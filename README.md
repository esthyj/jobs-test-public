...
- 직업 등급이 달라 등급이 낮은 직업코드(E로 갈수록 등급이 낮음)를 제시했습니다. 
...
재검증(`verify_result_20260421_145812.txt`) 결과, 검증 대상 41건 중 GT가 맞다고 판단된 케이스는 26건, Pred가 맞다고 판단된 케이스는 15건입니다. 단, Pred로 판정된 15건 중 `2_code`(질의 생성 시 사용했던 직업코드 설명 2개)에 포함되는 14건은 실제로는 정답 배열 내 다른 코드를 맞힌 것이므로 원래 순위를 유지합니다. 
...
- GPT-5.4가 GT/Pred를 판정한 경우를 보았을 때, (처음 몇개의 케이스를 확인했을 때) 전부 GPT-5.4의 판정이 맞아, 그냥 집계상 믿고 따르기로 했습니다. 
