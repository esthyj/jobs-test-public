...
| B | `B_job_classify_now` | 직업코드 설명 기반 데이터 생성 | 99.41% (`Recall@5`) |
...
| E | `E_job_classify_now` | 본사 데이터 + 직업코드 설명 기반 데이터 생성 | 97.7% (재검증 반영 최종 점수, Claude 기준)<br>94.04% (재검증 반영 최종 점수, Gemini 기준) |
...
└── result/
    ...
    │   └── E_job_classify_now/                 # 본사 데이터 + 직업코드 설명 기반 생성 데이터
    │       ├── claude/
    │       │   ├── questions_claude.jsonl
    │       │   ├── eval_result_20260420_145852_claude.txt
    │       │   └── verify_result_20260420_164804_claude.txt
    │       └── gemini/
    │           ├── questions_gemini.jsonl
    │           ├── eval_result_20260420_160521_gemini.txt
    │           └── verify_result_20260420_164310_gemini.txt
    └── compare_llm_models/
        ├── README.md                           # 모델별 결과 비교
...
- `Recall@1`: 추가 질문이 필요한 케이스를 놓치지 않고 잡아낸 비율입니다. 즉, "재질의가 필요하다"고 정답 라벨이 붙은 케이스 중 모델이 실제로 재질의가 필요하다고 판단한 비율입니다.
...
재검증(`verify_result_20260420_164804_claude.txt`) 결과, 검증 대상 18건 중 GT가 맞다고 판단된 케이스는 17건, Pred가 맞다고 판단된 케이스는 1건입니다.
...
재검증(`verify_result_20260420_164310_gemini.txt`) 결과, 검증 대상 51건 중 GT가 맞다고 판단된 케이스는 44건, Pred가 맞다고 판단된 케이스는 7건입니다.
